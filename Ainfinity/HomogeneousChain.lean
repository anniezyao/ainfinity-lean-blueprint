...
def total_deg {β : Type u} [Grading β] {obj : Type v} {gquiver : GQuiver.{u, v, w} β obj} {X : obj} {Y : obj} (chain : HomogeneousChain (gquiver := gquiver) X Y) : β :=
  match chain with
  | @Quiver.Path.nil obj (toQuiver gquiver) _ => (0 : β)
  | @Quiver.Path.cons obj (toQuiver gquiver) _ _ _ most ⟨d, mor⟩ => (total_deg most) + d

def correct_output_deg {β : Type u} [inst : Grading β] {obj : Type v} {gquiver : GQuiver.{u, v, w} β obj} {X : obj} {Y : obj} (chain : HomogeneousChain (gquiver := gquiver) X Y) : β :=
  let path : @Quiver.Path obj (toQuiver gquiver) X Y := chain
  let len := @Quiver.Path.length obj (toQuiver gquiver) X Y path
  total_deg chain + shift_ofInt (2 - (len : ℤ))
